...
export function CartItemList({
  ...
  const [activeCouponItemId, setActiveCouponItemId] = useState<number | null>(
    null,
  );

  const handleCouponSelectAndApply = (
              ...
              entries.map(([brand, items]) => (
                <BrandSection key={brand}>
                  {items.map((item, itemIndex) => {
                    const currentCoupon =
                      selectedCouponMap.get(item.id) || null;
                    ...
                    if (currentCoupon && currentCoupon.walletId !== null) {
                      const calculatedDiscount = Math.floor(
                        priceForCouponCalculation *
                          (currentCoupon.percent / 100),
                      );
                      itemCouponDiscount = Math.min(
                        calculatedDiscount,
                        currentCoupon.maxDiscountPrice,
                      );
                    }

                    const finalItemPrice =
                      priceForCouponCalculation - itemCouponDiscount;
